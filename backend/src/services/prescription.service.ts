...
export class PrescriptionService {
   ...
   */
  static async getById(prescriptionId: number) {
    const pool = getPool();

    // Query 1: Prescription main details
    const presResult = await pool.request()
      .input('prescriptionId', mssql.Int, prescriptionId)
      .query(`
        SELECT 
          pr.prescription_id,
          pr.diagnosis,
   ...
   */
  static async create(input: PrescriptionInput) {
    const pool = getPool();
    ...
    try {
      ...
      const presRequest = new mssql.Request(transaction);
      const presResult = await presRequest
        .input('patientId', mssql.Int, input.patient_id)
        .input('doctorId', mssql.Int, input.doctor_id)
        .input('diagnosis', mssql.VarChar, input.diagnosis)
        .query(`
      ...
      for (const item of input.medicines) {
        ...
        const medReq = new mssql.Request(transaction);
        const medCheck = await medReq
          .input('medicineId', mssql.Int, item.medicine_id)
          .query('SELECT medicine_name FROM dbo.Medicines WHERE medicine_id = @medicineId');
        
        if (!medCheck.recordset[0]) {
          throw new NotFoundError(`Medicine with ID ${item.medicine_id} does not exist.`);
        }
        ...
        await itemReq
          .input('prescriptionId', mssql.Int, prescriptionId)
          .input('medicineId', mssql.Int, item.medicine_id)
          .input('dosage', mssql.VarChar, item.dosage)
          .input('quantity', mssql.Int, item.quantity)
          .input('duration', mssql.VarChar, item.duration)
          .query(`
        ...
        if (invCheck.recordset[0]) {
          ...
          await updateInvReq
            .input('inventoryId', mssql.Int, invRecord.inventory_id)
            .input('newQuantity', mssql.Int, newQty)
      ...
      await activityReq
        .input('action', mssql.VarChar, 'Prescription Created')
        .input('description', mssql.VarChar, `Prescription issued to ${patientName} by ${doctorName}`)
